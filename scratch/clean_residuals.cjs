...
async function cleanAllResiduals() {
  const idsToDelete = [7, 9, 11, 12];
  for (const id of idsToDelete) {
    await supabase.from('alunos').delete().eq('id', id);
  }
  const { data: alunos } = await supabase.from('alunos').select('id, nome, cpf');
  console.log('📌 Alunos remanescentes no banco após deleção cirúrgica:', alunos);
}

cleanAllResiduals().catch(console.error);
