...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    // Opcionalmente incluir lista de alunos (para modal de confirmacao)
    let alunosDaTurma = [];
    if (req.query.incluirAlunos === '1') {
      let alunosQ = supabase
        ...
        .eq('turmaid', turmaId)
        .order('nome');
      alunosQ = applyInstituicaoFilter(alunosQ, instituicaoId);
      const { data: al } = await alunosQ;
      alunosDaTurma = al || [];
    }
