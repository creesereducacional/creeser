...
export default async function handler(req, res) {
  ...
  // Se for perfil professor, carregar o registro do diário de classe para verificar se a turma e a disciplina pertencem ao vinculo dele
  ...
  if (req.method === 'GET') {
    ...
    if (isProfessor) {
      ...
      if (vinErr || !vin) {
        return res.status(403).json({ error: 'Acesso negado: você não possui vínculo com este registro de planejamento diário.' });
      }
    }
  ...
  if (req.method === 'PUT') {
    ...
    if (isProfessor) {
      const { data: current, error: currentErr } = await supabase.from('planejamento_diario').select('turma, disciplina').eq('id', id).single();
