...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (method === 'PUT') {
      ...
      // Validação de unicidade de Matriz por Instituição na Edição (Fase 5.2)
      // Exclui a própria unidade (id !== self.id), permitindo que a matriz atual permaneça matriz
      if (payload.is_matriz && payload.instituicao_id) {
        ...
        if (errMatrizCheck) {
          console.error('Erro ao verificar matriz existente na edição:', errMatrizCheck);
          return res.status(500).json({ error: 'Erro ao verificar unicidade da matriz', detail: errMatrizCheck.message });
        }

        if (matrizExistente) {
          return res.status(409).json({
            error: `Conflito: A instituição selecionada já possui outra Unidade Matriz cadastrada ("${matrizExistente.nome}"). Não é permitido cadastrar mais de uma matriz para a mesma instituição.`,
          });
        }
      }
