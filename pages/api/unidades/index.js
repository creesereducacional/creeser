  ...
  try {
    if (req.method === 'GET') {
      ...
      if (error) {
        ...
        return res.status(500).json({ error: 'Erro ao recuperar unidades', detail: error.message });
      }
    ...
    if (req.method === 'POST') {
      ...
      if (error) {
        ...
        return res.status(500).json({ error: 'Erro ao criar unidade', detail: error.message });
      }

      try {
        await syncCursosUnidade(data[0].id, body.cursos);
      } catch (syncError) {
        console.error('Supabase curso_unidade error:', syncError);
        return res.status(500).json({ error: 'Erro ao vincular cursos da unidade', detail: syncError.message });
      }
