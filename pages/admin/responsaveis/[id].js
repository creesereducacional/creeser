...
export default function EditarResponsavel() {
  ...
  const carregarResponsavel = async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        const alunoIds = Array.isArray(data.alunoIds)
          ? data.alunoIds
          : Array.isArray(data.alunoids)
            ? data.alunoids
            : [];

        setAlunosSelecionados(alunoIds.map((value) => String(value)));
      ...
      alert('Erro ao carregar responsável');
    } finally {
      setLoading(false);
    }
  };
  ...
  const removerAluno = (alunoId) => {
    setAlunosSelecionados((prev) => prev.filter((idSelecionado) => idSelecionado !== alunoId));
  };
      ...
      <div className="p-4 md:p-6 max-w-5xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-gray-800">Editar Responsável</h1>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-md p-4 md:p-6">
            <h3 className="text-lg font-bold text-teal-600 mb-4">Dados Pessoais</h3>
          ...
          <div className="bg-white rounded-lg shadow-md p-4 md:p-6">
            ...
            <div className="mb-4">
              ...
              <input
                ...
                className="w-full px-3 py-2 text-sm border border-teal-300 rounded-lg focus:outline-none focus:border-teal-500 bg-teal-50"
              />
              <datalist id="alunos-busca-edicao">
