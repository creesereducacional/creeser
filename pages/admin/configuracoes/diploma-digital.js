...
export default function DiplomaDigital() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('/api/configuracoes/diploma-digital', {
        method: 'POST',
      ...
      if (res.ok) {
        alert('Configurações de diploma salvas com sucesso');
      } else {
          ...
          <div className="bg-white rounded-lg shadow-md p-4 md:p-6">
            <label className="flex items-center gap-3 cursor-pointer">
              <input
                ...
                className="w-5 h-5"
              />
              <span className="text-sm font-medium text-gray-800">Diploma Digital ATIVO</span>
            </label>
          </div>
