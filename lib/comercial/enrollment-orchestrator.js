...
export async function efetuarMatriculaVenda(supabase, vendaId, dadosMatricula = {}) {
  ...
  try {
    ...
    const valorMensalidade = parseFloat(dadosMatricula.valor_mensalidade || curso?.valor_mensalidade || 300.00);
    const qtdParcelas = parseInt(dadosMatricula.qtd_parcelas || curso?.qtd_parcelas || 12);
    const diaPagamento = parseInt(dadosMatricula.dia_pagamento || 10);
