...
const findContratoPadraoInstituicao = async (instituicaoId) => {
  ...
  if (instituicaoId) {
    const { data: cInst, error: errInst } = await supabase
      .from('contratos_instituicao')
      .select('*')
      .or(`instituicao_id.eq.${instituicaoId},instituicaoid.eq.${instituicaoId}`)
      .eq('ativo', true)
      .order('padrao', { ascending: false })
      .order('ordem', { ascending: true })
      .order('nome', { ascending: true })
    ...
    if (!errInst && cInst && cInst.length > 0) {
      return cInst[0];
    }
  }

  // Fallback: se não encontrar específico por ID, pegar o primeiro contrato ativo marcado como padrão globalmente
  const { data: dataGlobal } = await query.limit(1);
  return (dataGlobal || [])[0] || null;
};
...
export class ContratoResolverService {
  ...
  static async resolveContratoAluno(alunoId, options = {}) {
    ...
    const instituicaoIdCandidato = aluno.instituicao_id || aluno.instituicaoid || turma?.instituicao_id || turma?.instituicaoid;
    const instituicao =
      (await findInstituicaoPorId(instituicaoIdCandidato)) ||
      (await findInstituicaoPorNome(aluno)) ||
      (turma?.instituicao ? await findInstituicaoPorNome({ instituicao: turma.instituicao }) : null);
    ...
    if (!contrato) {
      // Tentar por instituicao.id ou fallback para os primeiros contratos ativos cadastrados se houver apenas uma instituicao
    ...
    const placeholders = {
      ...
      '{{ALUNO_NACIONALIDADE}}': asText(aluno.nacionalidade || (aluno.pais_origem?.includes('BRA') ? 'BRASILEIRA' : aluno.pais_origem) || 'BRASILEIRA'),
      '{{ALUNO_NATURALIDADE}}': asText(aluno.naturalidade ? (aluno.uf_naturalidade && !aluno.naturalidade.includes('-') ? `${aluno.naturalidade} - ${aluno.uf_naturalidade}` : aluno.naturalidade) : ''),
      '{{ALUNO_ESTADO_CIVIL}}': asText(aluno.estadocivil || aluno.estado_civil || aluno.estadoCivil),
