 ...
 *  - Consulta OK e sem linhas → retorna []  (usuário legado sem vínculo → fallback legado permitido)
 *  - Consulta OK e com linhas → retorna array normalizado
 *  - Consulta com erro        → LANÇA erro (não transforma erro em ausência de vínculo)
 *
 * O FK hint 'unidades!usuario_instituicoes_unidade_id_fkey' corresponde ao nome
 * gerado automaticamente pelo PostgreSQL para a FK declarada sem nome explícito
 * na migration 20260917152048. Não alterar sem confirmar no banco.
 *
...
export const getUserVinculos = async (usuarioId) => {
  ...
  if (error) {
    // Lançar em vez de silenciar: o chamador decide o comportamento seguro.
    // Retornar [] aqui seria equivalente a dizer "sem vínculos" e ativar
    // legacyFallback, potencialmente ampliando escopo em caso de falha de banco.
    const msg = `[auth-server] getUserVinculos falhou para usuario_id=${usuarioId}: ${error.message}`;
    console.error(msg);
    throw new Error(msg);
  }
 ...
 *    legacyFallback:       boolean,        — true se usou usuarios.instituicao_id (sem vínculo em usuario_instituicoes)
 *    queryError:           null,
 *  }
 *
 * Retorno em caso de erro de banco (getUserVinculos lançou):
 *  {
 *    queryError: string,  — mensagem de erro
 *    ...demais campos null/false
 *  }
 *  O endpoint deve verificar queryError e retornar 503, nunca ampliar escopo.
 *
...
export const resolveContextoUsuario = async (req, authUser) => {
  ...
  if (isGroupAdmin) {
    ...
    return {
      // undefined → applyInstituicaoFilter ignora o filtro (retorna todos)
      // null seria interpretado como WHERE instituicao_id IS NULL (incorreto)
      instituicaoId:      requestedInstId || undefined,
  ...
  // Buscar vínculos reais do usuário — pode lançar se o banco falhar
  let vinculos;
  try {
    vinculos = await getUserVinculos(authUser?.id);
  } catch (err) {
    // Erro de banco: NÃO ativar fallback legado (isso ampliaria escopo indevidamente).
    // Devolver objeto de erro explícito para o endpoint decidir (ex: retornar 503).
    return {
      instituicaoId:      null,
      unidadesPermitidas: null,
      isGroupAdmin:       false,
  ...
  if (vinculos.length > 0) {
    ...
    // Consulta OK mas sem vínculos: usuário legado → fallback permitido
    legacyFallback = true;
