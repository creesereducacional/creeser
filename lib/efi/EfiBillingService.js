 ...
 * @throws {Error} Lança erro em caso de falha — o chamador é responsável pelo rollback
 */
export async function emitirBoletoEfi({
  parcela,
  aluno,
  descricao,
  configFinanceiro = {},
  ...
  if (notificationUrl) {
    await efi
      ...
      .catch((err) =>
        console.warn('[EfiBillingService] Falha ao definir notification_url:', err.message)
      );
  }
  ...
  try {
    ...
    console.error(
      '[EfiBillingService] Falha ao associar boleto na EFI. Solicitando cancelamento da cobrança EFI:',
      chargeId,
      billetErr.message
    );
    try {
