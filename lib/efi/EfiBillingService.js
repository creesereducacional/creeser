...
async function emitirBoletoEfi({
  ...
  // ── 6. Desconto por pontualidade / Desconto Incondicional ──
  const discountValue = Number(parcela.valor_desconto) || 0;
  let discount = null;
  let conditionalDiscount = null;

  if (discountValue > 0) {
    if (parcela.data_limite_desconto) {
      conditionalDiscount = {
        type: 'currency',
        value: Math.round(discountValue * 100),
        until_date: parcela.data_limite_desconto,
      };
    } else {
      discount = {
        type: 'currency',
  ...
  if (conditionalDiscount) {
    bankingBilletPayload.conditional_discount = conditionalDiscount;
  }
