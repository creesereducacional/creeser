...
/**
 * Converte qualquer valor num string legível — nunca retorna objeto.
 * @param {*} v
 * @returns {string|null}
 */
function toStr(v) {
...
function resolverMensagemErro(err) {
  ...
  if (typeof err === 'string') return err || 'Erro desconhecido';
  if (err instanceof Error) {
    const efiResp = err.efiResponse;
    if (efiResp) {
      // Cada campo pode ser objeto — usar toStr() para garantir string
      return (
        toStr(efiResp.error_description) ||
        toStr(efiResp.message) ||
        toStr(efiResp.error) ||
...
async function emitirBoletoEfi({
  ...
  // ── 6. Desconto por pontualidade ──
  // NOTA: O endpoint POST /v1/charge/{id}/pay da EFI aceita discount apenas com
  // { type, value } — o campo until_date NÃO faz parte do schema deste endpoint
  // e causa rejeição 422. A data_limite_desconto é salva no banco (controle interno)
  // mas não é enviada ao gateway neste fluxo.
  const discountValue = Number(parcela.valor_desconto) || 0;
  const discount = {};
  if (discountValue > 0) {
    discount.value = Math.round(discountValue * 100);
    discount.type = 'currency';
    // until_date removido — não suportado pelo schema do banking_billet
  }
  ...
  let billetData;
  try {
    billetData = await efi.payBillet(chargeId, bankingBilletPayload);
  } catch (billetErr) {
