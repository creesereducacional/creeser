...
async function _request(method, path, body = null) {
  ...
  if (!res.ok) {
    // A EFI pode retornar data.message como objeto em alguns erros de validacao.
    // Garantir que a mensagem seja sempre uma string legivel.
    const rawMessage = data.message ?? data.error ?? data.error_description ?? null;
...
/**
 * Associa a cobrança a um boleto bancário (definePayMethod) — payload básico.
...
async function associateBillet(chargeId, payload) {
  // A EFI usa POST /charge/:id/pay com estrutura { payment: { banking_billet: { expire_at, customer } } }
  return _request('POST', `/v1/charge/${chargeId}/pay`, {
    payment: {
      banking_billet: {
...
/**
 * Associa a cobrança a um boleto bancário com payload completo.
 * Suporta configurations (multa/juros) e discount (desconto condicional).
 * @param {number} chargeId          — ID retornado por createCharge
 * @param {Object} bankingBilletPayload — payload completo para banking_billet
 *   {
 *     expire_at        : 'YYYY-MM-DD',
 *     customer         : { name, cpf, ... },
 *     configurations   : { fine: { value, type }, interest: { value, type } },  // opcional
 *     discount         : { value, type, until_date },                           // opcional
 *   }
 */
async function payBillet(chargeId, bankingBilletPayload) {
  return _request('POST', `/v1/charge/${chargeId}/pay`, {
